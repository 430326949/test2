...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	for (i = 0; i < data_seq.length(); ++i) {
		if (data_seq[i].x == 5 && data_seq[i].y == 6 && std::strcmp(data_seq[i].color, "black") == 0)
		{
			
			if (UserDataTypeTypeSupport::print_data(&data_seq[i]))
			{
				//测试接收一次消息会写入5次
				recivecount++;
			...
			if (recivecount %5 == 0) {
				std::cout << "receice thougthout:" << data_seq[i].StructSize()*recivecount / 5 * 8 << std::endl;
			}
			boolListen = 1;

			std::string input = std::to_string(data_seq[i].x) + std::to_string(data_seq[i].y) + data_seq[i].color;
...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	instance->x = 3;
	instance->y = 4;
	instance->color = const_cast<char *>("redddddd");
