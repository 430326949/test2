...
std::string md5(const std::string& str) {
	...
	char res[2 * EVP_MAX_MD_SIZE + 1];
	memset(res, 0, sizeof(res)); // 首先将缓冲区清零
	for (int i = 0; i < md_len; ++i)
		sprintf_s(&res[i * 2], sizeof(res) - i * 2, "%02x", (unsigned int)md_value[i]);
...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	for (i = 0; i < data_seq.length(); ++i) {
		if (data_seq[i].x == 3 && data_seq[i].y == 4 && std::strcmp(data_seq[i].color, "redddddd") == 0)
		{
			//std::cout << "reserve                 dddd:" << data_seq[i].StructSize() << std::endl;
...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	instance->x = 5;
	instance->y = 6;
	instance->color = const_cast<char *>("black");
	...
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		if (boolListen == 1)
		{
			...
			{
				//发送吞吐量
				std::cout << "throughput                      " << instance->StructSize()*count*8  << std::endl;
				//	std::cout << "throughput                      " << sizeof(instance->x)+ sizeof(instance->y)<<" " << strlen(const_cast<char *>("redddddd"))<< std::endl;
				...
				//MD5
				std::string input = std::to_string(instance->x) + std::to_string(instance->y) + instance->color;
				std::string output = md5(input);
				std::cout << "MD5 hash of \"" << input << "\" is: " << output << std::endl;
